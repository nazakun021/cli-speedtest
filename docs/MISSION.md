...
- **Adaptive Fallback**: The engine must detect rate-limiting (HTTP 429) and automatically fallback to a single-connection model to ensure test completion.
...
- **Sample Count**: Minimum 20 samples per test.
- **Pacing**: 50ms interval between samples to avoid ICMP-style rate limiting.
- **Metrics**: Min, Max, Average, Jitter (Average Deviation), and Packet Loss.
...
The "brain" of the tool. It manages the test sequence (Ping → Download → Upload), handles configuration aggregation, and performs the final mathematical reductions. It is strictly decoupled from CLI argument parsing to support library-level integration and exhaustive automated testing.

### Measurement Engine (`client.rs`)
The high-concurrency I/O layer. It manages the lifecycle of `tokio` tasks, handles streaming byte counters using atomic primitives, and implements the retry/retry-after logic. It is responsible for translating raw network responses into measurement samples.
